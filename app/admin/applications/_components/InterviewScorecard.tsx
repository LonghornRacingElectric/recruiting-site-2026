...
export default function InterviewScorecard({
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/applications/${applicationId}/interview-scorecard${systemParam}`)
      .then(res => res.json())
      .then(data => {
      ...
      .catch(err => console.error("Failed to fetch interview scorecard", err))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
        await fetch(`/api/admin/applications/${applicationId}/interview-scorecard`, {
            method: "POST",
        ...
        toast.error("Failed to save interview scorecard");
    } finally {
        setSaving(false);
    }
  };
  ...
  if (loading && !scorecardConfig) {
    return (
      <div className="flex items-center gap-2 py-4">
        <Loader2 className="h-4 w-4 animate-spin" style={{ color: "var(--lhr-blue)" }} />
        <span className="font-urbanist text-[13px] text-white/30">Loading interview scorecard...</span>
      </div>
    );
  }
  ...
  if (!loading && !scorecardConfig) {
    return (
      <div
        ...
        style={{ backgroundColor: "rgba(4,95,133,0.06)", border: "1px solid rgba(4,95,133,0.15)" }}
      >
        <p className="font-urbanist text-[13px]" style={{ color: "rgba(4,95,133,0.8)" }}>
  ...
  return (
    <div className="max-w-2xl space-y-6">
        ...
        {isPrivilegedUser && allTeamSystems.length > 1 && (
            ...
            >
                <label className="block font-urbanist text-[10px] font-semibold tracking-widest uppercase mb-2" style={{ color: "var(--lhr-gray-blue)" }}>
                    Select System Interview Scorecard
                </label>
                ...
                <p className="font-urbanist text-[10px] text-white/15 mt-2">
                    <span style={{ color: "rgba(34,197,94,0.7)" }}>●</span> indicates systems with configured interview scorecards
                </p>
            </div>
        )}
        ...
        {aggregates && aggregates.totalSubmissions > 0 && (
            <div
              className="p-4 rounded-xl"
              style={{ backgroundColor: "rgba(4,95,133,0.04)", border: "1px solid rgba(4,95,133,0.12)" }}
            >
                <div className="flex items-center justify-between mb-4">
                    <h3 className="font-montserrat text-[13px] font-bold text-white">Interview Aggregate Scores</h3>
                    <span className="font-urbanist text-[12px] text-white/30">
                        {aggregates.totalSubmissions} interviewer{aggregates.totalSubmissions !== 1 ? 's' : ''}
                    </span>
                </div>
                ...
                {aggregates.overallWeightedAverage !== undefined && (
                    ...
                    >
                        <div className="flex items-center justify-between">
                            ...
                            <span className="font-montserrat text-[22px] font-bold" style={{ color: "var(--lhr-blue)" }}>
                            ...
                            <div className="flex-1">
                                <div className="flex items-center justify-between mb-1">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className="font-urbanist text-[12px] font-semibold" style={{ color: "var(--lhr-blue)" }}>
                                ...
                                <div className="h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: "rgba(255,255,255,0.04)" }}>
                                    <div
                                        ...
                                        style={{ width: `${(score.average / score.max) * 100}%`, background: "linear-gradient(90deg, var(--lhr-blue), #0891b2)" }}
                                    />
                                </div>
                            </div>
                        </div>
        ...
        {/* Individual Interviewer Submissions */}
        {allSubmissions.length > 0 && (
            <div
              className="p-4 rounded-xl"
              style={{ backgroundColor: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)" }}
            >
                <h3 className="font-montserrat text-[13px] font-bold text-white mb-4">Interviewer Submissions</h3>
                <div className="space-y-3">
        ...
        {/* My Interview Scorecard Form */}
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.04)" }} className="pt-6">
            <h3 className="font-montserrat text-[13px] font-bold text-white mb-4">Your Interview Scorecard</h3>
            <form onSubmit={handleSubmit} className="space-y-5">
                                        ...
                                        <div
                                          ...
                                          style={
                                            scorecardData[field.id] === val
                                              ? { backgroundColor: "var(--lhr-blue)", color: "white", border: "1px solid var(--lhr-blue)" }
                ...
                <div className="flex justify-end">
                    ...
                    >
                        ...
                        {saving ? "Saving..." : "Save Interview Scorecard"}
                    </button>
                </div>
            </form>
        </div>
