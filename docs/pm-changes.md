...
| 23 | Remove LinkedIn from the contact page (keep it elsewhere, e.g. footer) | **done** | Contact page is now config-driven (#26); the seed content has Instagram only. LinkedIn stays in the footer. Admins can re-add channels any time. |
| 24 | Contact page copy: email must be `longhornracingrecruitment@gmail.com` ("Contact with any questions about the recruiting process"), IG blurb "follow for live updates on all recruiting events" | **done** | Baked into the config seed for #26 — live immediately since `config/contact_page` starts non-existent (seed = live behavior per the getDefaultX rule). |
...
| 26 | Contact page fields editable in the admin panel | **done** | New `config/contact_page` doc, `GET /api/contact` (15 min cache), admin `GET/PUT /api/admin/config/contact`, and an admin-only Contact tab (intro, email + blurb, social channels add/edit/remove, CTA heading/text). Contact page converted to a server component — content now renders in the initial HTML (SEO win, closes its item in the code queue). |
| 27 | Remove "rolling admissions" claims — applications are not rolling | **done** | The contact CTA now reads "Don't wait until the deadline — apply now...". The `config/teams` scan hit turned out to be **"rolling resistance"** in Solar's Dynamics subsystem description — a legitimate engineering term, left alone. No other rolling claims in code or live config. |
| 28 | Electric "Software" is formally "Vehicle Modeling & Software" | **done (2026-08-04)** | Enum value renamed; `slugifySystem` now strips ampersands so doc IDs/URL params stay clean (`vehicle-modeling-software`). Live migration: 6 staff `memberProfile.system` values, `interviewConfigs/electric-software` moved to the new slug, `config/teams` subsystem display name (backup in scratchpad). Old test applications keep the old string — irrelevant, #2 wipes them. **Note:** an orphan `interviewConfigs/electric-vehicle-modeling` doc predates this (the system was once renamed to "VMod" and reverted — see `scripts/migrate-systems.ts`); left in place, deletable from the Interviews tab. |
