...
| 19 | Trial workday email includes the system applied to | **needs info (asked PM)** | Already works: live `trial_offered` template has `{{systemNames}}`, populated from `trialOffers`. Real gap is the `\|\| "General"` fallback in `templates.ts:38` when the offer list is empty — hit 1 of 173 trial emails, 1 of 209 interview emails. Asked PM whether she saw a "General" email or wants ranked `preferredSystems` instead. |
